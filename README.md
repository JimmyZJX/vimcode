...
- Current validation:
  ...
  - `npm test -- --runInBand` passes with 545 enabled tests (28 skipped).
...
- motions: `h`, `j`, `k`, `l`, `space`, `w`, `W`, `e`, `E`, `b`, `B`, `ge`, `gE`, `0`, `^`, `_`, `$`, `+`, `-`, `enter`, `gg`, `G`, `g_`, `f`, `F`, `t`, `T`, `;`, `,`, `%`, counted `%`, `]}`, `])`, `[{`, `[(`, `}`, `{`, `)`, `(`, and local mark jumps
...
- `/...<enter>` and `?...<enter>` regex search with smart-case matching, `n`/`N` repeat, `*`/`#` word search (case-insensitive regardless of the word's case — Vim `:h star`: 'ignorecase' applies, 'smartcase' does not; visual `*`/`#` follow the same rule), and search-as-operator ranges through a VSCode-native search capability boundary; pending search input uses a small single-line editor for cursor navigation, deletion, and clipboard paste while VSCode find decorations are updated without revealing the native Find widget, seeded with the last query while empty, and cleared after the Vim search commits
  - Vim `search-offset` character offsets (`/pat/e`, `/pat/s`, `/pat/b`, each with an optional `+N`/`-N` delta; `e` targets the end of the match and makes an operator motion inclusive, `s`/`b` the start) are parsed off the query, carried on `n`/`N`/`*`/`#` repeats (which re-apply the offset and still make progress), and applied through the same search `Motion` so normal, operator, and visual paths share one code path. Line offsets (`/pat/2`) are not yet supported. This is a Vim feature Zed lacks, so it has no upstream fixture; `src/vim/search_offset.test.ts` pins the behavior
- first surround operators: `ys`, `yss`, `ds`, `cs`, and visual `S` for word/motion/quote/bracket ranges; `ys`/`ds`/`cs` (including the tag-entry forms) are dot-repeatable like vim-surround with repeat.vim, `yss` wraps only the addressed line (never the trailing newline), and charwise wraps strip trailing whitespace out of the wrap (`ysw)` on `foo bar` gives `(foo) bar`)
- replay undo granularity: dot replays are wrapped in one undo transaction like macro replays, so the host's word-boundary undo stops cannot split a `.` repeat into multiple undo units; live insert sessions deliberately keep the host's native undo granularity (VSCode's word-level typing undo), diverging from Vim's one-unit-per-insert-session
- native Insert-mode paste (`vim.insertModeCtrlVAsPaste`, default true): `ctrl-v` runs VS Code's `editor.action.clipboardPasteAction` instead of Vim literal insertion; Visual Block `ctrl-v` is unchanged
- VSCodeVim-compatible `vim.highlightedyank.{enable,color,textColor,duration}` (default off): yanks flash the yanked ranges — charwise, linewise, and per-row visual block — via a host decoration for the configured duration
- VSCodeVim-compatible `vim.statusBarColorControl` + `vim.statusBarColors.*` (default off, VSCodeVim default colors): the status bar recolors per Vim mode through the same `workbench.colorCustomizations` keys VSCodeVim writes, but applied to the window's in-memory configuration layer — settings.json is never touched and disabling restores the user's colors
- optional VSCodeVim ReplaceWithRegister (`vim.replaceWithRegister`, default false): `gr{motion}`/`grr` and visual `gr`, with counts, named registers, linewise targets, register preservation, and dot repeat; when disabled, `grr`/`grn`/`gra` remain LSP bindings
...
- first macro slice: `q{register}` recording, `@{register}` / `@@` replay, counted replay, and `Q` replay-last for focused fixtures; `@:` repeats the last executed command line (`:h quote_:`), counted and `@@`-repeatable, shared across editors, E30 when there is none
...
- `:g` framework prerequisites: full `:g[lobal]`/`:g!`/`:v[global]` spellings with Vim's delimiter rules (`:v!` rejected like E477); `:g`/`:s` share the last search pattern with `/` (`:g//`/`:s//` reuse it — E35-quiet without one — and explicit patterns set `@/` and what `n` follows, even matchless, but deliberately not the visible search highlight: an editing command's pattern must not glow after e.g. `:g/pat/s//X/` + undo); `:g/pat/normal` executes on marks that track edits (Neovim-faithful `LineTracker`: insertions/deletions shift pending marks, deleted or merged lines skip theirs) while `:[range]normal` iterates fixed rows; either form is a single undo step restoring the cursor to the first executed line
- `:g` sub-command dispatch: every other tail runs as a full ex command per tracked mark with the cursor on the mark, so `:g/pat/s/…/…/` (including `:g/pat/s//…/` reusing the `:g` pattern), `:g/^/m0` (buffer reverse), `:g/pat/t$`, `:g/pat/j`, `:g/pat/pu`, and per-mark relative ranges (`:g/x/.,+1d`) compose from the existing commands in one undo step; bare `:g/pat` moves the cursor to the last match (`:p` without output); whole-tail `:d[elete]` keeps the batched single-edit path; nested `:g` is a no-op like Vim's `global_busy`
- `:[range]d[elete] [reg] [count]` and `:[range]y[ank] [reg] [count]` with Vim register semantics (unnamed + `"1` rotation for deletes — even named ones, `"0` for unnamed yanks, uppercase append; `:y` leaves the cursor); `:g/pat/d` rotates one delete per mark and `qaq` + `:g/pat/y A` collects matches (`q` now mirrors recorded macros into the registers, so a register that exists-but-empty appends with its separator like Neovim)
- `:[range]>[>…] [count]` / `:[range]<[<…]` ex indent, one shiftwidth per repetition, cursor on the last line's first non-blank; composes with `:g/pat/>`
- Vim 'shiftwidth' follows the host's resolved indent size (VSCode `indentSize`/auto-detection), so `>>`/`<<`/`v>`/`:>` shift by the same amount as the editor's own indent commands; visual `[count]>` multiplies the shift (`v2>`), the count survives dot repeat, and the `v_>` cursor keeps its column unshifted, clamped (Neovim-probed)
- ex-command outcome feedback in the status bar (Vim `:h 'report'`-gated): `3 fewer lines`/`more lines`, `4 substitutions on 3 lines`, `:s` `n`-flag counting (`3 matches on 2 lines`, aggregated across a `:g` run like Vim's `do_sub` totals), `N lines yanked`, and warning-styled `Pattern not found: …` (E486) / `No previous regular expression` (E35)
- live `:g` inline preview through the substitute-preview channel: typing `:g/pat` highlights the match on every line pass 1 would mark (whole line for `:v`), a substitute tail previews its replacements on exactly those lines (empty tail pattern = the `:g` pattern), and a `:d` tail previews whole-line deletions
...
5. **Plugin-style integrations.** Implement compatibility plugins as modules over the core and adapter capabilities, not as one-off key hacks. Priority order: highlighted yank, EasyMotion, Sneak, Commentary, CamelCaseMotion/subword motions, indent/argument/entire text objects.
...
4. **Plugin-style integrations are important but later.** Keep EasyMotion, Sneak, HighlightedYank, Commentary, CamelCaseMotion/subword motions, and extra text objects in the backlog. Implement them as modules over explicit adapter capabilities rather than one-off key hacks.
