...
- first surround operators: `ys`, `yss`, `ds`, `cs`, and visual `S` for word/motion/quote/bracket ranges; `ys`/`ds`/`cs` (including the tag-entry forms) are dot-repeatable like vim-surround with repeat.vim, and `yss` wraps only the addressed line (never the trailing newline)
